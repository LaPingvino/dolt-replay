...
4. **RenameColumn (dlite-source)**: doltlite's `dolt_schema_diff` system table returns `Error: unknown operation` when the only schema change between two refs is a column rename (probed 2026-05-04 against system doltlite — `dolt_schema_diff` only handles ADD/DROP-shaped operations). The actual rename does apply correctly inside doltlite (pragma_table_info confirms). Fix options on dolt-replay side: when `dolt_schema_diff` returns the `unknown operation` error, fall back to fetching root→parent and root→child to_create_statements separately and compute the diff client-side. Better: have doltlite handle rename in dolt_schema_diff. Dolt-source side already works since `dolt diff -r sql` natively emits RENAME COLUMN.
